...
class LinearCRF(nn.Module):
    ...
    def add_constraint_for_iobes(self, transition: torch.Tensor):
        ...
        for prev_label in self.idx2labels:
            ...
            for next_label in self.idx2labels:
                if (
                    next_label == START_TAG
                    or next_label == STOP_TAG
                    or next_label == PAD
                ):
                    continue
                if prev_label == "O" and (
                    next_label.startswith("I-") or next_label.startswith("E-")
                ):
                    transition[
                        self.label2idx[prev_label], self.label2idx[next_label]
                    ] = -10000.0
    ...
    def get_marginal_score(
        self, lstm_scores: torch.Tensor, word_seq_lens: torch.Tensor
    ) -> torch.Tensor:
        marginal = self.forward_backward(
            lstm_scores=lstm_scores, word_seq_lens=word_seq_lens
        )
        return marginal
    ...
    def backward(
        ...
        rev_score = self.transition.transpose(0, 1).view(
            1, 1, self.label_size, self.label_size
        ).expand(
        ...
        last_beta += (
            self.transition.transpose(0, 1)[:, self.start_idx]
            .view(1, self.label_size)
            .expand(batch_size, self.label_size)
        )
        last_beta = log_sum_exp_pytorch(
            last_beta.view(batch_size, self.label_size, 1)
        ).view(batch_size)
    ...
    def forward_labeled(
        ...
        currentTagScores = torch.gather(
            all_scores,
            3,
            tags.view(batchSize, sentLength, 1, 1).expand(
                batchSize, sentLength, self.label_size, 1
            ),
        ...
        if sentLength != 1:
            tagTransScoresMiddle = torch.gather(
                currentTagScores[:, 1:, :],
                2,
                tags[:, : sentLength - 1].view(batchSize, sentLength - 1, 1),
            ).view(batchSize, -1)
        ...
        tagTransScoresEnd = torch.gather(
            self.transition[:, self.end_idx]
            .view(1, self.label_size)
            .expand(batchSize, self.label_size),
            1,
            endTagIds,
        ).view(batchSize)
    ...
    def viterbi_decode(
        ...
        scoresRecord = torch.zeros(
            [batchSize, sentLength, self.label_size], device=curr_dev
        )
        idxRecord = torch.zeros(
            [batchSize, sentLength, self.label_size], dtype=torch.int64, device=curr_dev
        )
        ...
        for wordIdx in range(1, sentLength):
            ...
            scoresRecord[:, wordIdx, :] = torch.gather(
                scoresIdx,
                1,
                idxRecord[:, wordIdx, :].view(batchSize, 1, self.label_size),
            ).view(batchSize, self.label_size)

        lastScores = torch.gather(
            scoresRecord,
            1,
            word_seq_lens.view(batchSize, 1, 1).expand(batchSize, 1, self.label_size)
            - 1,
        ).view(batchSize, self.label_size)  ##select position
        lastScores += (
            self.transition[:, self.end_idx]
            .view(1, self.label_size)
            .expand(batchSize, self.label_size)
        ...
        for distance2Last in range(sentLength - 1):
            lastNIdxRecord = torch.gather(
                ...
                1,
                torch.where(
                    word_seq_lens - distance2Last - 1 > 0,
                    word_seq_lens - distance2Last - 1,
                    mask,
                )
                .view(batchSize, 1, 1)
                .expand(batchSize, 1, self.label_size),
            ).view(batchSize, self.label_size)
            decodeIdx[:, distance2Last + 1] = torch.gather(
                lastNIdxRecord, 1, decodeIdx[:, distance2Last].view(batchSize, 1)
            ).view(batchSize)
...
if __name__ == "__main__":
    ...
    model = LinearCRF(label_size=len(labels), label2idx=label2idx, idx2labels=labels)
    seq_len = 80
    batch_size = 5
    ...
    with torch.no_grad():
        scores, indices = model.viterbi_decode(
            all_scores=all_scores, word_seq_lens=word_seq_lens
        )
    print(scores.squeeze(-1))
    for idx in range(batch_size):
        indices[idx][: word_seq_lens[idx]] = indices[idx][: word_seq_lens[idx]].flip(
            [0]
        )
